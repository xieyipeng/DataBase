...
//        List<String> cityCode=separate();
//        List<String> city=getCity(cityCode);
//        List<String> list1=new ArrayList<>();
//        List<String> list2=new ArrayList<>();
//        for (int i = 0; i < city.size(); i++) {
//            if (list1.contains(city.get(i))){
//                System.out.println(i);
//                list2.add(city.get(i));
//            }else {
//                list1.add(city.get(i));
//            }
//        }
//        System.out.println(list2);
     ...
     * 将分割好的城市进一步分解为单个城市
     *
     * @param allCity 一次分割的城市信息
     * @return 城市代码
     */
    public static List<String> getCode(List<String> allCity) {
        List<String> city = new ArrayList<>();
        for (String string :
                allCity) {
     ...
     * @return 城市名称
     */
    public static List<String> getCity(List<String> allCity) {
        List<String> city = new ArrayList<>();
        for (String string :
                allCity) {
    ...
    /**
     * 全国城市信息切割为单个城市及其对应代码编号
     *
     * @return 单个城市信息
     */
    public static List<String> separate() {
        List<String> cityCode = new ArrayList<>();
        final String weatherCity = "(\\D*):(\\d*)";
        Pattern patternCity = Pattern.compile(weatherCity);
        Matcher matcherCity = patternCity.matcher(CITY_CODE);
        while (matcherCity.find()) {
            cityCode.add(matcherCity.group());
        }
        return cityCode;
    }
     ...
     * @param city 城市
     */
    private static int getCode(String city) {
        int code;
        if (CITY_CODE.contains(city)) {
            int n = CITY_CODE.indexOf(city);
            String result = CITY_CODE.substring(n + 3, n + 12);
